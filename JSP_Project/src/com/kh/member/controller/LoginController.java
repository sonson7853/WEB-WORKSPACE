...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 응답 페이지에 전달한 값이 있을 경우 값을 어딘가에 담아야함(담아줄수 있는 Servlet scope의 내장 객체 4종류)
		 * 
		...
		if(loginUser == null) { // 로그인 실패 => 에러페이지 응답
		...
		}else { // 로그인 성공 => index 페이지(메인페이지) 응답
			
			// 로그인한 회원의 정보를 로그아웃하기 전까지 계속 갖다가 쓸 것이기 때문에 session에 데이터 담기
			
			// Servlet 에서 JSP내장객체인 SESSION에 접근하고자 한다면 우선 session 객체를 얻어와야함.
			...
			//1. 포워딩 방식 응답 뷰 출력하기
			//	 선택된 jsp가 보여질 뿐 url에는 여전히 현재 이 서블릿에 대한 매핑값이 남아 있을 것
			//	 localhost:8082/jspproject/login.me
			//   url에 내용이 그대로 남아있는 상태에서 새로고침(f5)누르게 되면 계속 로그인 요청을 보내게 될것.
