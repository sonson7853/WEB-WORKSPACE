...
<body>
   ...
   <!-- 
      * JSP를 이루는 구성인자
      ...
      3. JSP 액션태그 : XML 기술을 이용해서 기존 jsp 문법을 확장하는 기술을 제공하는 태그
         - 표준액션태그(Standard Action Tag) : JSP 페이지에서 바로 사용가능(별도의 라이브러리가 필요 없다)
                                   표준액션태그는 모든 태그명 앞에 jsp: 이라는 접두어가 붙음.
         - 커스텀액션태그(Custom Action Tag) : JSP 페이지에서 바로 사용 불가능(별도의 라이브러리 연동 해줘야함)
   ...
   <p>
      XML 기술을 이용해서 기존 JSP문법을 확장시키는 기술을 제공하는 태그들
   </p>
   
   <h3>1. 표준 액션 태그</h3>
   <p>
      JSP 페이지에서 별도의 라이브러리 연동 없이 바로 사용 가능함<br>
      태그 앞에 jsp:접두어가 붙음
   </p>
